...
(function () {
    ...
    const PaginationController = {
        currentPage: 1,
        itemsPerPage: 6, // Default, will be recalculated
        totalItems: 0,
        totalPages: 0,
        gridElement: null,
        cardElements: [],
        paginationNumbersContainer: null,
        prevButton: null,
        nextButton: null,
        debounceTimer: null, // For resize debouncing

        init() {
            this.gridElement = Utils.getElement(".destinations-grid");
            this.paginationNumbersContainer = Utils.getElement(
                ".pagination-numbers"
            );
            this.prevButton = Utils.getElement(".pagination-arrow.prev-page");
            this.nextButton = Utils.getElement(".pagination-arrow.next-page");

            if (
                !this.gridElement ||
                !this.paginationNumbersContainer ||
                !this.prevButton ||
                !this.nextButton
            ) {
                // console.log("Pagination elements not found, skipping initialization.");
                return; // Exit if essential elements are missing
            }

            // Select only direct children .destination-card elements
            this.cardElements = Array.from(this.gridElement.children).filter(
                (el) => el.classList.contains("destination-card")
            );
            this.totalItems = this.cardElements.length;

            if (this.totalItems === 0) {
                console.log("No destination cards found for pagination.");
                return;
            }
        ...
        calculateItemsPerPage() {
            if (!this.gridElement) return 6; // Default if grid not found

            const gridWidth = this.gridElement.offsetWidth;
            // Use the minmax value from CSS: minmax(280px, 1fr)
            const cardMinWidth = 280;
            // Get gap from CSS variable or default
            const gapStyle = getComputedStyle(this.gridElement).gap;
            const gap = parseInt(gapStyle) || 32; // Default gap if not found

            const effectiveCardWidth = cardMinWidth + gap;
            const columns = Math.max(
                1,
                Math.floor(gridWidth / effectiveCardWidth)
            );

            // User wants exactly 2 rows
            this.itemsPerPage = columns * 2;
            // console.log(`GridWidth: ${gridWidth}, Columns: ${columns}, ItemsPerPage: ${this.itemsPerPage}`);
            return this.itemsPerPage;
        },

        displayPage(page) {
            this.currentPage = page;
            // Ensure totalPages is up-to-date, especially after resize
            ...
            // const scrollY = window.scrollY; // REMOVE THIS LINE

            this.cardElements.forEach((card, index) => {
                // Use 'visible' class to control display based on CSS
            ...
            this.updateArrowButtons();
            this.renderPaginationNumbers(); // Re-render numbers to update active state
        ...
        setupPagination() {
            this.itemsPerPage = this.calculateItemsPerPage(); // Recalculate based on current width
            this.totalPages = Math.ceil(this.totalItems / this.itemsPerPage);
        ...
        renderPaginationNumbers() {
            if (!this.paginationNumbersContainer) return;
            this.paginationNumbersContainer.innerHTML = ""; // Clear existing numbers

            const maxVisibleNumbers = 3; // User requirement

            // Helper to create a button or ellipsis
            const createControl = (
                text,
                pageNum = null,
                isActive = false,
                ...
                if (isEllipsis) {
                    ...
                    if (pageNum !== null) {
                        // Add preventDefault here for number buttons
                        button.addEventListener("click", (e) => {
            ...
            // --- Keep the rest of the rendering logic the same as before ---
            if (this.totalPages <= maxVisibleNumbers + 2) {
                // Show all if 5 or less
                for (let i = 1; i <= this.totalPages; i++) {
                    this.paginationNumbersContainer.appendChild(
                        createControl(i, i, i === this.currentPage)
                    );
                }
            } else {
                // Always show first page
                this.paginationNumbersContainer.appendChild(
                    createControl(1, 1, 1 === this.currentPage)
                );

                // Logic for ellipsis and middle numbers
                let startNum, endNum;

                if (this.currentPage <= maxVisibleNumbers) {
                    // Near the start
                    startNum = 2;
                    endNum = maxVisibleNumbers;
                    this.paginationNumbersContainer.appendChild(
                        createControl(
                            startNum,
                            startNum,
                    ...
                    this.paginationNumbersContainer.appendChild(
                        createControl(
                            endNum,
                            endNum,
                            endNum === this.currentPage
                        )
                    );
                    this.paginationNumbersContainer.appendChild(
                        createControl("...", null, false, true)
                    ); // Ellipsis end
                } else if (
                    this.currentPage >
                    this.totalPages - maxVisibleNumbers + 1
                ) {
                    // Near the end
                    startNum = this.totalPages - maxVisibleNumbers + 1;
                    endNum = this.totalPages - 1;
                    this.paginationNumbersContainer.appendChild(
                        createControl("...", null, false, true)
                    ); // Ellipsis start
                    this.paginationNumbersContainer.appendChild(
                    ...
                    // In the middle
                    startNum = this.currentPage - 1;
                    endNum = this.currentPage + 1;
                    this.paginationNumbersContainer.appendChild(
                        createControl("...", null, false, true)
                    ); // Ellipsis start
                    this.paginationNumbersContainer.appendChild(
                    ...
                    this.paginationNumbersContainer.appendChild(
                        createControl(this.currentPage, this.currentPage, true)
                    );
                    this.paginationNumbersContainer.appendChild(
                        createControl(
                            endNum,
        ...
        updateArrowButtons() {
            ...
            if (this.currentPage <= 1) {
                ...
                this.prevButton.setAttribute("disabled", "true"); // Also set disabled attribute
            } else {
            ...
            if (this.currentPage >= this.totalPages) {
                ...
                this.nextButton.setAttribute("disabled", "true"); // Also set disabled attribute
            } else {
        ...
        addEventListeners() {
            ...
            // Modify the number button listener generation within renderPaginationNumbers
            // to also include preventDefault

            // Debounced resize handler (keep as is from previous step)
            window.addEventListener("resize", () => {
                clearTimeout(this.debounceTimer);
                this.debounceTimer = setTimeout(() => {
                    const previousItemsPerPage = this.itemsPerPage;
                    this.itemsPerPage = this.calculateItemsPerPage(); // Recalculate
                    ...
                    if (previousItemsPerPage !== this.itemsPerPage) {
                        ...
                        this.renderPaginationNumbers(); // This needs to re-add listeners with preventDefault
                        this.displayPage(this.currentPage);
                    }
                }, 250);
            });
    ...
    document.addEventListener("DOMContentLoaded", function () {
        async function initializeApp() {
            try {
                ...
                // Initialize Pagination Controller *after* main content might be loaded
